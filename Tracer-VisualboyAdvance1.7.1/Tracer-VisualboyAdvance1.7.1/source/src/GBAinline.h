...
#include <stdio.h>
extern u32 rd_low,rd_high,wr_low,wr_high;
extern void Trace_Hook(char Format[], u32 addr, u32 value);
extern FILE* fp_hook;
...
{
  if(fp_hook && addr>=(rd_low&(~0x03)) && addr<=(rd_high|0x03))
    Trace_Hook("[%02X:%06X] R32 = %08X [%08X]",addr,READ32LE(((u32*)&map[(addr)>>24].address[(addr) & map[(addr)>>24].mask])));
...
inline u32 CPUReadMemory(u32 address)
{
  ...
  if(fp_hook && address>=(rd_low&(~0x03)) && address<=(rd_high|0x03))
    Trace_Hook("[%02X:%06X] R32 = %08X [%08X]",address,value);
...
inline u32 CPUReadHalfWord(u32 address)
{
  ...
  if(fp_hook && address>=(rd_low&(~0x01)) && address<=(rd_high|0x01))
    Trace_Hook("[%02X:%06X] R16 = %04X     [%08X]",address,value);
...
{
	if(fp_hook && address>=rd_low && address<=rd_high)
    Trace_Hook("[%02X:%06X] R08            [%08X]",address,0);
...
{
  if(fp_hook && address>=(wr_low&(~0x03)) && address<=(wr_high|0x03))
    Trace_Hook("[%02X:%06X] W32 = %08X [%08X]",address,value);
