...
FILE *fp_hook=0, *fp_hook2;
u32 rd_mode=-1,rd_low=-1,rd_high=-1;
u32 wr_mode=-1,wr_low=-1,wr_high=-1;
u32 pc_mode=-1,pc_low=-1,pc_high=-1;
bool hook_trace=0;
  ...
  if(type == IMAGE_GB) {
    ...
    if(theApp.autoIPS) {
      ...
      //utilApplyIPS(ipsname, &gbRom, &size);
			extern void ApplyIPS(const char *ips, u8 **image, int *size);
			ApplyIPS(theApp.szFile,&gbRom,&size);
      if(size != gbRomSize) {
    ...
    */
    ...
    if(theApp.autoIPS) {
      ...
      utilApplyIPS(ipsname, &rom, &size);
			//extern void ApplyIPS(const char *ips, u8 **image, int *size);
			//ApplyIPS(theApp.szFile,&rom,&size);
      if(size != 0x2000000) {
        CPUReset();
      }
    }
...
///////////////////////////////////////////////////
  ...
  if(fp1) {
		fscanf(fp1,"hook_pc1 %x %x %x\n",&pc_mode,&pc_low,&pc_high);
		fscanf(fp1,"hook_rd1 %x %x %x\n",&rd_mode,&rd_low,&rd_high);
		fscanf(fp1,"hook_wr1 %x %x %x\n",&wr_mode,&wr_low,&wr_high);

    fp_hook2 = fopen("hook.txt","w");
		fclose( fp1 );
	}
...
{
///////////////////////////////////////////////////
		if(pMsg->message == WM_KEYDOWN && pMsg->wParam == 188)
		{
			// ',' = memory hooking
			hook_trace = hook_trace ? 0:1;
		...
		if(pMsg->message == WM_KEYDOWN && pMsg->wParam == 190)
		{
			// '.' = dump WRAM + IRAM + VRAM
			FILE* fp;
			...
			// Work RAM
			if( gbWram ) {
				fp = fopen("gb_wram.bin","wb");
				fwrite(gbWram,1,0x8000,fp);
				fclose( fp );
			}
			else if( workRAM ) {
				fp = fopen("gba_wram.bin","wb");
				fwrite(workRAM,1,0x40000,fp);
				fclose(fp);
			}
			else {
				fp = fopen("cgb_wram.bin","wb");
				fwrite( gbMemoryMap[0xc],1,0x2000,fp );
				fclose( fp );
			}

			// Palette RAM
			if( gbPalette ) {
				fp = fopen("cram.gb_pal","wb");
				fwrite(gbPalette,1,2*128,fp);
				fclose(fp);
			}
			else if( paletteRAM ) {
				fp = fopen("cram.gba_pal","wb");
				fwrite(paletteRAM,1,0x400,fp);
				fclose(fp);
			}

			// Super Gameboy
			extern u8 *gbSgbBorderChar;
			extern u16 gbSgbSCPPalette[4*512];

			if( gbSgbBorderChar ) {
				fp = fopen( "sgb_border.bin", "wb" );
				fwrite(	gbSgbBorderChar, 1, 32*256, fp );
				fclose( fp );

				fp = fopen( "cram.sgb_pal", "wb" );
				fwrite(	gbSgbSCPPalette, 1, 512*4, fp );
				fclose( fp );
			}
			...
			if(gbRam)
				fwrite(gbRam,1,gbRamSize,fp);
			fclose(fp);

			// Internal RAM
			if(internalRAM)	{
				fp = fopen("gba_ram.bin","wb");
				fwrite(internalRAM,1,0x8000,fp);
				fclose( fp );
			}
			else if( gbMemory ) {
				fp = fopen("gb_ram.bin","wb");
				fwrite(gbMemory+0xa000,1,0x2000,fp);
				fclose(fp);
			}

			// Video RAM
			if( gbVram ) {
				fp = fopen("gbc_vram.bin","wb");
				fwrite(gbVram,1,0x4000,fp);
				fclose( fp );
			}
			else if( vram ) {
				fp = fopen("gba_vram.bin","wb");
				fwrite(vram,1,0x18000,fp);
				fclose( fp );
			}
			else {
				fp = fopen("gb_vram.bin","wb");
				fwrite(gbMemory+0x8000,1,0x2000,fp);
				fclose(fp);
			}
      ...
      // BG info
			if( workRAM ) {
				...
				fprintf(fp,"%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n",
					"Source: GBATek",
					"MODE  # of BGs  Colors/Palettes   Screen",
					"0     4 = 0123  16/16 or 256/1    Tile/Map",
					"1     3 = 012-  16/16 or 256/1    Tile/Map",
					"2     2 = --23  256/1             Tile/Map",
					"3     1 = --?-  32768             Bitmap",
					"4     2 = --??  256/1             Bitmap",
					"5     2 = --??  32768             Bitmap");
				fprintf(fp,"Mode: %01X\n",DISPCNT&0x7);
				fprintf(fp,"BG%d: Colors:%3d, VScroll:%03X, HScroll:%03X, W:%d, H:%d, NT=60:%04X, TA=60:%04X\n",
					0, (BG0CNT>>7)&1?256:16, BG0VOFS, BG0HOFS, ((BG0CNT >> 14) & 1) * 32 + 32, ((BG0CNT >> 15) & 1) * 32 + 32, ((BG0CNT>>8)&0x1f) * 0x800,  ((BG0CNT>>2)&0x03) * 0x4000);
				fprintf(fp,"BG%d: Colors:%3d, VScroll:%03X, HScroll:%03X, W:%d, H:%d, NT=60:%04X, TA=60:%04X\n",
					1, (BG1CNT>>7)&1?256:16, BG1VOFS, BG1HOFS, ((BG1CNT >> 14) & 1) * 32 + 32, ((BG1CNT >> 15) & 1) * 32 + 32, ((BG1CNT>>8)&0x1f) * 0x800,  ((BG1CNT>>2)&0x03) * 0x4000);
				fprintf(fp,"BG%d: Colors:%3d, VScroll:%03X, HScroll:%03X, W:%d, H:%d, NT=60:%04X, TA=60:%04X\n",
					2, (BG2CNT>>7)&1?256:16, BG2VOFS, BG2HOFS, ((BG2CNT >> 14) & 1) * 32 + 32, ((BG2CNT >> 15) & 1) * 32 + 32, ((BG2CNT>>8)&0x1f) * 0x800,  ((BG2CNT>>2)&0x03) * 0x4000);
				fprintf(fp,"BG%d: Colors:%3d, VScroll:%03X, HScroll:%03X, W:%d, H:%d, NT=60:%04X, TA=60:%04X\n",
					3, (BG3CNT>>7)&1?256:16, BG3VOFS, BG3HOFS, ((BG3CNT >> 14) & 1) * 32 + 32, ((BG3CNT >> 15) & 1) * 32 + 32, ((BG3CNT>>8)&0x1f) * 0x800,  ((BG3CNT>>2)&0x03) * 0x4000);
				/*
				fprintf(fp,"OBJ: Select:%01X, NT:0x%04X, TA:0x%04X\n",
					PPU.OBJNameSelect,PPU.OAMAddr<<1,PPU.OBJNameBase);
				*/
				fclose(fp);
			}
		}
		if(pMsg->message == WM_KEYDOWN && pMsg->wParam == 191)
		{
			// '/' = trace
			extern bool map_trace;
			extern FILE* trace;
			map_trace = map_trace ? 0:1;
			if(!map_trace && trace)
				fprintf(trace,"\n%s\n\n","TRACE STOPPED");
		}
///////////////////////////////////////////////////
