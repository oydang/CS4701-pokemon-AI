...
*/
	...
	// (fixes ATI Radeon 340M issue)
	while ( ddsOffscreen->Lock( NULL, &ddsDesc, DDLOCK_DONOTWAIT | DDLOCK_WRITEONLY | DDLOCK_SURFACEMEMORYPTR, NULL ) != DD_OK && 
					ddsOffscreen->IsLost() != DDERR_SURFACELOST ) {
		Sleep (10);
	}
	hret = ddsOffscreen->IsLost();
    ...
    if(theApp.tripleBuffering && theApp.videoOption > VIDEO_4X) {
      ...
      if(hret == DD_OK) {
        if(theApp.menuToggle || !theApp.active) {
          pDirectDraw->FlipToGDISurface();
          ddsPrimary->SetClipper(ddsClipper);
          hret = ddsPrimary->Blt(&theApp.dest, ddsFlip, NULL, DDBLT_WAIT, NULL);
      ...
      hret = ddsPrimary->Blt(&theApp.dest, ddsOffscreen, NULL,DDBLT_WAIT,NULL);
